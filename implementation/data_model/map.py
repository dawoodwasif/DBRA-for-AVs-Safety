...
class Map(object):
    ...
    def get_distance_to_lane_end(self, location: Location) -> float:
        ...
        if lane is None:
            print(f"Warning: No lane found for the given location: {location}")
            return offset * float(one_meter_as_longitudinal_offset)
    ...
    def get_point_on_lane_left(self, location: Location) -> Location:
        ...
        if current_lane is None:
            print(f"Warning: No current lane found for the given location: {location}")
            return None

        if left_lane is None:
            print(f"Warning: No left lane found for the given location: {location}")
            return None
    ...
    def __get_lane_left(self, location: Location) -> Optional[ad.map.lane.Lane]:
        ...
        ----------
        location : Location
            location on the lane where the left lane is searched for

        Returns
        -------
        Optional[ad.map.lane.Lane]:
            lane to the left of the given lane, or None if no valid left lane exists
        """
        lane: ad.map.lane.Lane = self.__get_lane(location)
        ...
        if lane.direction == ad.map.lane.LaneDirection.NEGATIVE:
            # Get lanes to the right (since it's opposite in NEGATIVE direction)
            left_lanes = ad.map.lane.getContactLanes(lane, ad.map.lane.ContactLocation.RIGHT)
        else:
            # Get lanes to the left
        ...
        if left_lane is None:
            print(f"Warning: No valid left lane exists for the lane at location: {location}")
            return None
    ...
    def __get_lane(self, location: Location) -> ad.map.lane.Lane:
        ...
        if len(map_matching_result) == 0:
            print("No valid lane found for the given location.")
            return None
        else:
    ...
    def get_next_point_on_lane(self, start_point: Location, distance: float) -> Location:
        ...
        # Check if map_matching_result is empty before proceeding
        if not map_matching_result:
            print(f"Warning: No valid lane found for the given location: {start_point}")
            # Return a fallback value or handle it as needed
            return Location(0, 0, 0)  # Or return None or handle it according to your logic
