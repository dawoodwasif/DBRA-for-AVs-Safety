...
def prune_irrelevant_nodes(evidence_nodes: Dict[str, List[Outcome]]) -> Dict[str, List[Outcome]]:
    """
    Prune nodes with low impact or irrelevant nodes to reduce complexity.
    In this example, we keep only nodes with high outcome probability (> 0.5).
    """
    pruned_evidence_nodes = {}
    
    # Simple pruning logic: Keep nodes where the maximum outcome value is greater than 0.5
    for node_id, outcomes in evidence_nodes.items():
        max_outcome_value = max([outcome.value for outcome in outcomes])
        if max_outcome_value > 0.5:  # Keep nodes with significant outcomes
            pruned_evidence_nodes[node_id] = outcomes
    
    return pruned_evidence_nodes
...
class BayesianNetworkInference:
    ...
    def _inference_query_and_build_infered_bn_output(bn_instance: "BayesianModel", output_nodes: List[str],
        ...
        # Apply complexity reduction (prune nodes) if the flag is enabled
        if complexity_reduction:
            evidence_query = prune_irrelevant_nodes(evidence_query)
